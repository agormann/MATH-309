...
% The entries in the eigenvector X2 are sorted in ascending order, and the
% entries in eigenvectors X3, X4, X5 are sorted according to X2.
% Input:
...
[~, I] = sort(vec(:, 1), 'ascend'); % sort according to X2
vec = vec(I, :);
x = 1:h;
...
title(tiledPlot, {"Components of x_i", "Sorted According to x_2"})
xlabel(tiledPlot, "Sorted Component"); ylabel(tiledPlot, "Magnitude");

t1 = nexttile(); % X1 plot
plot(x, vec(:, 1), '.-');
title(t1, "Components of x_2");
yline(t1, 0, '--');
xlim(t1, [0 h]);

t2 = nexttile; % X2 plot
plot(x, vec(:, 2), '.-');
title(t2, "Components of x_3");
yline(t2, 0, '--');
xlim(t2, [0 h]);

t3 = nexttile; % X3 plot
plot(x, vec(:, 3), '.-');
title(t3, "Components of x_4");
yline(t3, 0, '--');
xlim(t3, [0 h]);

t4 = nexttile; % X4 plot
plot(x, vec(:, 4), '.-');
title(t4, "Components of x_5");
yline(t4, 0, '--');
xlim(t4, [0 h]);
